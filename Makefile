...
wca_package_in_docker_with_kafka:
	...
	# target: standalone
	sudo docker build --no-cache --network host -f Dockerfile.kafka -t $(WCA_IMAGE):$(WCA_TAG) .
	# Extract pex to dist folder
	rm -rf .cidfile && sudo docker create --cidfile=.cidfile $(WCA_IMAGE):$(WCA_TAG)
	CID=$$(cat .cidfile); \
	mkdir -p dist; \
...
wca_docker_devel:
	...
	sudo docker build --no-cache --network host --target devel -f Dockerfile -t $(REPO)$(WCA_IMAGE):$(WCA_TAG) .
